...
export class RectRenderEngine extends BaseRenderEngine {
  ...
  private drawInactiveRect(labelRect: LabelRect, data: EditorData) {
    ...
    const labelName: LabelName = LabelsSelector.getLabelNameById(
      labelRect.labelId
    );
    this.renderRect(
      rectOnImage,
  ...
  private drawActiveRect(labelRect: LabelRect, data: EditorData) {
    ...
    this.renderRect(rectOnImage, true, lineColor, anchorColor, labelName?.name);
  }
  ...
  private renderRect(
    ...
    if (labelName) {
      const labelPosition: IPoint = {
        x: rectOnImage.x + 35, // Offset slightly inside the rectangle
        y: rectOnImage.y + 15,
      };
      DrawUtil.drawText(
        this.canvas,
  ...
  private getRectUnderMouse(data: EditorData): LabelRect {
    ...
    for (const labelRect of labelRects) {
      if (
        labelRect.isVisible &&
        this.isMouseOverRectEdges(labelRect.rect, data)
      ) {
        return labelRect;
      }
    }
